...
/*
cat/proc/slabinfo命令的显示结构主要由三部分组成：
statistics部分显示的是slabcache的基本统计信息，这部分是我们最常用的，下面是每一列
的含义：
active_objs表示slab cache中已经被分配出去的对象个数
num_objs表示slab cache中容纳的对象总数
objsize表示slab中对象的object size，单位为字节
objperslab表示slab中可以容纳的对象个数
pagesperslab表示slab所需要的物理内存页个数
tunables部分显示的slab cache的动态可调节参数，如果我们采用的slub实现，那么tunables
部分全是0，/proc/slabinfo文件不可写，无法动态修改相关参数。如果我们使用的slab实现
的话，可以通过#echo 'name limit batch count  shared factor' > /proc/slabinfo命令动
态修改相关参数。命令中指定的name就是kmem_cache结构中的name属性。tunables这部分显示
的信息均是slab实现中的相关字段，只做简单了解即可，与我们本文主题slub的实现没有关系。
limit表示在slab的实现中，slab cache的cpu本地缓存array_cache最大可以容纳的对象个数
batch count表示当array_cache中缓存的对象不够时，需要一次性填充的空闲对象个数。
slab data部分显示的slab cache的总体信息，其中active_slabs一列展示的slab cache中活跃
的slab个数。nums_slabs一列展示的是slab cache中管理的slab总数
在cat /proc/slabinfo命令显示的这些系统中所有的slab cache，内核会将这些slab cache用一
个双向链表统一串联起来。链表的头结点指针保存在struct kmem_cache结构的list中。
*/
