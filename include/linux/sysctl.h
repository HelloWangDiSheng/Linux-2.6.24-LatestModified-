...
struct ctl_table 
{	
	/*数字ID，在该sysctl项所在的层次上必须是唯一的，但不必在整个sysctl表中唯一*/
	int ctl_name;
	/*/proc/sys下目录项的可理解信息或者为空*/
	const char *procname;	
	/*可指定任何值，通常是一个函数指针或字符串，字符串有特定于sysctl的函数处理，通用
	代码部分不会访问该成员*/
	void *data;
	/*指定了一个sysctl能够接收或输出的数据的最大长度（按字节计算）*/
	int maxlen;
	/*数据的访问权限，确定数据是否可读/写，谁可以读/写。权限时使用虚拟文件系统的常数指定*/
	mode_t mode;
	/*指向数组的指针，各个数组项都是ctl_table实例，这些clt_table实例是当前数据项的子节点*/
	...
	/*可以填充特定于proc的数据，通用的sysctl代码处理不会处理这两者，它们通常用于定义数值
	参数的上下限*/
	void *extra1, *extra2;
};
