...
{
	/*最大的预读数量，单位时PAGE_CACHE_UNIT*/
	unsigned long ra_pages;
	/*经常使用原子操作的设备状态*/
	unsigned long state;
	/*设备存储器的有关信息，如设备是否可以直接执行，对基于ROM的文件系统时必要的。但该
	信息中最重要的时页是否可以回写，对真正的块设备这总是可以的，但对基于内存的设备如
	RAM磁盘时不可能的，因为从内存向内存回写数据没什么意义，BDI_CAP_NO_WRITEBACK不需要
	数据同步*/
 ...
 */
 /*backing_dev_info中capability标识值*/
