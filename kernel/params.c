...
/*负责解释启动时传递到内核的命令行参数。在用户空间中也会遇到同样的问题，这是固有的，
必须将一个字符串包含“键/值”对的字符串分解。该字符串形如键1=值1 键2=值2。所设置的选
项必须保存到内核中，或触发特定的响应。内核不仅在启动时会遇到解析参数的问题，在插入
模块时也会遇到。因而它使用了同样的机制来解决相关问题，从而避免了不必要的代码复制*/
int parse_args(const char *name, char *args, struct kernel_param *params, unsigned num,
